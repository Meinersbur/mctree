...
CFLAGS=(-I"${CLANG_PREFIX}/projects/openmp/runtime/src" -I"${CLANG_PREFIX}/runtimes/runtimes-bins/openmp/runtime/src" -L"${CLANG_PREFIX}/runtimes/runtimes-bins/openmp/runtime/src" \
  ...
  -I"${SCRIPTPATH}" -DSW4_CROUTINES -DDEBUG -mllvm -polly -mllvm -polly-position=early -mllvm -polly-only-func=rhs4sg_rev_kernel  -mllvm -polly-process-unprofitable -mllvm -polly-allow-nonaffine-branches -mllvm -polly-print-instructions -mllvm -polly-use-llvm-names -mllvm -polly-process-unprofitable -mllvm -debug-only=polly-detect,polly-scops -mllvm -polly-ignore-aliasing)
