...
(cd "${SCRIPTPATH}" && python3 "${ROOTPATH}/mctree.py" autotune --ld-library-path="${CLANG_PREFIX}/lib:${CLANG_PREFIX}/runtimes/runtimes-bins/openmp/runtime/src" \
  ...
  *.c -o "${BASENAME}" \
  -I"/usr/include/x86_64-linux-gnu/mpich" -lmpich -lm -fcommon \
